...
export function sSelection(args: Inputs) {
  ...
  for (const item of args.top
    ...
    if (item.timePoints) {
      const timeSimilarityBoost = item.timePoints.reduce((boost, timePoint) => {
        const [hours, minutes] = timePoint.split(":").map(Number);
        const timePointMinutes = hours * 60 + minutes;
        const difference = Math.abs(currentMinutes - timePointMinutes);

        // Apply boost if within 1 hour (60 minutes)
        return difference <= 60 ? boost + (1 - difference / 60) : boost;
      }, 0);
    ...
    const timeSinceLastPlayed =
      Date.now() - new Date(item.lastPlayedAt).getTime();
    const decayFactor = Math.exp(
      -timeSinceLastPlayed / (1000 * 60 * 60 * 24 * 7)
    ); // Decay daily
    item.weight *= decayFactor;

    const timeSinceAdded = Date.now() - new Date(item.lastPlayedAt).getTime();
