...
function readVideo(item: any): SearchVideo | undefined {
  try {
    ...
    if (
      ...
      video.badges[0].metadataBadgeRenderer.style == "BADGE_STYLE_TYPE_LIVE_NOW"
    ) {
      isLive = true;
    }
    ...
    return {
      ...
      channelTitle:
        video.ownerText && video.ownerText.runs
...
async function getYoutubePlayerDetails(url: string) {
  try {
    ...
    const ytInitialData = pageContent.split("var ytInitialPlayerResponse =");
...
async function nextPage(
  ...
  try {
    ...
    const pageContent = (await page.json()) as any;

    const data =
      pageContent.onResponseReceivedCommands[0].appendContinuationItemsAction;
    ...
    for (const conitem of data.continuationItems) {
      ...
      for (const item of conitem.itemSectionRenderer.contents) {
        ...
        if (
          ...
          const playlistData = await getPlaylistData(playlistRen.playlistId);
...
async function getPlaylistData(id: string, limit: number = 0) {
  ...
  try {
    ...
    const videoItems =
      sectionRen.contents.twoColumnBrowseResultsRenderer.tabs[0].tabRenderer
